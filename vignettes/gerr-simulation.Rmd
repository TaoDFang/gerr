...
Finally, we merge key results of the simulation studies into one figure. We show the particular case of $p_g=0.01$. The conclusions however are valid for other settings of $p_g$.

```{r oneFigure, echo=FALSE, fig.width=14, fig.height=9}
addPanelName <- function(plot, text) {
  textg <- textGrob(text, x=unit(0, "npc"), y=unit(1, "npc"), 
                    just=c("left", "top"),
                    gp=gpar(col="black", fontsize=18))
  compactPlot <- plot + theme(
    plot.margin = unit(c(0,0,0,0), "cm"),
    axis.title.x = element_text(margin=margin(0,0,0,0)), 
    axis.title.y = element_text(margin=margin(0,0,0,0)), 
    plot.title = element_text(margin=margin(0,0,0,0)))
  res <- arrangeGrob(compactPlot,
                     top=textg)
  return(res)
}
pA <- addPanelName(veriFpPlot,"A")
pB <- addPanelName(oeDenPlot,
                  "B")
pC <- addPanelName(tprGerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Sensitivity [gerr]"),
                  "C")
pD <- addPanelName(fprGerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Specificity [gerr]"),
                  "D")
pE <- addPanelName(f1Gerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+%
                    ggtitle(expression(F[1]~score~"[gerr]")),
                  "E")
pF <- addPanelName(tprFF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Sensitivity [FET+FDR]"),
                  "F")
pG <- addPanelName(fprFF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+%
                    ggtitle("Specificity [FET+FDR]"),
                  "G")
pH <- addPanelName(f1FF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle(expression(F[1]~score~"[FET+FDR]")),
                  "H")
pI <- addPanelName(tprDiff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Difference"),
                  "I")
pJ <- addPanelName(fprDiff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Difference"),
                  "J")
pK <- addPanelName(f1Diff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("Difference"),
                  "K")
layoutMatrix <- matrix(c(NA,3,4,5,
                         1,3,4,5,
                         1,6,7,8,
                         2,6,7,8,
                         2,9,10,11,
                         NA,9,10,11), byrow=TRUE, ncol=4)
grid.arrange(grobs=list(pA, pB, pC, pD, pE, pF, pG, pH, pI, pJ, pK),
