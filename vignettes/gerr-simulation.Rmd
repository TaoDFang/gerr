...
tprDiff <- ggplot(avgFullProbSimRes, aes(x=factor(p_gs), y=factor(p_g), fill=TPR.gerr-TPR.ff)) +
  ...
  scale_fill_gradient2(low=diffLowCol, mid=diffMidCol, high=diffHighCol, midpoint=0,
                       ...
                       name=expression(Delta~Sensitivity)) +
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
...
fprGerr <- ggplot(avgFullProbSimRes, 
  ...
  facet_wrap(~p_n, labeller = label_both) +
  geom_tile() +
  scale_fill_gradient2(low=lowCol, mid=midCol, high=highCol, name="Specificity",
                       midpoint = 0.5,
...
f1FF <- ggplot(avgFullProbSimRes, 
  ...
  geom_tile() +
  scale_fill_gradient2(low=lowCol, mid=midCol, high=highCol, 
                       name=expression(F[1]~score), limits=c(0,1), 
                       oob=scales::squish) +
  theme(axis.text.x = element_text(angle=45, hjust=1)) +
...
myGrob <- function(text) {
  textGrob(text, x=unit(0, "npc"), y=unit(1, "npc"), just=c("left", "top"),
           gp=gpar(col="black", fontsize=18))
}
pA <- arrangeGrob(veriFpPlot,
                  top=myGrob("A"))
pB <- arrangeGrob(oeDenPlot,
                  top=myGrob("B"))
pC <- arrangeGrob(tprGerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("gerr"),
                  top=myGrob("C"))
pD <- arrangeGrob(fprGerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("gerr"),
                  top=myGrob("D"))
pE <- arrangeGrob(f1Gerr %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+%
                    ggtitle("gerr"),
                  top=myGrob("E"))
pF <- arrangeGrob(tprFF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("FET+FDR"),
                  top=myGrob("F"))
pG <- arrangeGrob(fprFF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+%
                    ggtitle("FET+FDR"),
                  top=myGrob("G"))
pH <- arrangeGrob(f1FF %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle("FET+FDR"),
                  top=myGrob("H"))
pI <- arrangeGrob(tprDiff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle(""),
                  top=myGrob("I"))
pJ <- arrangeGrob(fprDiff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle(""),
                  top=myGrob("J"))
pK <- arrangeGrob(f1Diff %+% dplyr::filter(avgFullProbSimRes, p_n==0.001) %+% 
                    ggtitle(""),
                  top=myGrob("K"))
