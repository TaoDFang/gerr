...
if(!file.exists(modelVerifRdata) || params$overwriteCache) {
  ...
  verifKeyRes <- list(isRecovered = verifRes$isRecovered,
                      ...
                      selected_pathways_names=lapply(verifRes$results,
...
```{r verficateRes}
summary(verifKeyRes$isRecovered) ## all recovered
```
...
verifMt1AvgOeDen <- density(verifMt1AvgOe, from=0, to=1)
verifMt1AvgOeDenData <- data.frame(x=verifMt1AvgOeDen$x,
                                   y=verifMt1AvgOeDen$y/max(verifMt1AvgOeDen$y))
...
randomAvgOeDen <- density(randomAvgOe, from=0, to=1)
randomAvgOeDenData <- data.frame(x=randomAvgOeDen$x,
                                 y=randomAvgOeDen$y/max(randomAvgOeDen$y))
oeDen <- rbind(cbind(Class="gerr", verifMt1AvgOeDenData),
               cbind(Class="random", randomAvgOeDenData))
ggplot(oeDen,
       aes(x=x, y=y, color=Class)) +
  scale_colour_manual(values=c("red", "darkgray"),
                      name="") +
  geom_line(lwd=1.5)  + 
  ylab("Normalised density") +
  xlab("Average overlap coefficient")
```

We verify that all gene-sets that were assigned as genes of interest was successfully recovered. In 44 out of 500 gene-sets, more than one gene-set was selected by `gerr`. The average pairwise overlap coefficient between false-positive hits and the true positive hit is quite high (median `r median(verifMt1AvgOe)`, MAD `r mad(verifMt1AvgOe)`). It suggests that `gerr` is able to identify gene-sets that are highly enriched among many gene-sets.
...
The lack of sensitivity in the case of $p_{gs}<0.005$ is because we have only 500 gene-sets, and in random sampling, there are cases where no gene-set was selected to contribute to GOI.
